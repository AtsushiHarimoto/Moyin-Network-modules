...
# @moyin/net-client -- 日本語ドキュメント

フレームワーク非依存のTypeScript HTTPクライアントライブラリ。リクエスト重複排除、同時実行ポリシー、リトライ戦略、ストリーミングをサポートします。

## 概要

実際のアプリケーションでは、単純なHTTPリクエスト以上の課題に直面します:

- **重複リクエスト**: ダブルクリックや入力中の連続検索による無駄なリクエスト
- **古いレスポンス**: 新しいリクエスト完了後に到着する古いレスポンス
- **不安定なネットワーク**: 指数バックオフによるインテリジェントなリトライが必要
- **ローディング状態の散在**: コンポーネント間に分散する状態管理

`@moyin/net-client`はこれらすべてをトランスポート層で解決します。

## 4つの同時実行ポリシー

| ポリシー | 動作 | ユースケース |
|----------|------|-------------|
| `parallel` | すべてのリクエストが独立して実行 | バッチ操作 |
| `takeLatest` | 新しいリクエストが前のリクエストを自動キャンセル | インクリメンタル検索 |
| `dedupe` | 同一キーの後続呼び出しが既存のPromiseを共有 | コンポーネントマウント時 |
| `serial` | リクエストがキューに入り順次実行 | フォーム送信 |
...
const msg = resolveNetMessage('net.timeout', 'ja')
// => 'リクエストがタイムアウトしました。しばらくしてから再試行してください'
```

対応ロケール: `en` / `ja` / `zh-TW` / `zh-CN` / `zh-HK`

## Moyin Ecosystemの一部

このモジュールは[**Moyin Factory**](https://github.com/AtsushiHarimoto/Moyin-Factory)の一部です。
...
實際應用中會遇到單純 HTTP 請求以外的挑戰：

- **重複請求**：雙擊或連續搜尋輸入產生的無效請求
- **過期回應**：新請求完成後才到達的舊回應
- **不穩定網路**：需要指數退避的智慧重試
- **載入狀態散落**：分散在各組件中的狀態管理

`@moyin/net-client` 在傳輸層解決以上所有問題。

## 四種並行策略

| 策略 | 行為 | 使用場景 |
|------|------|---------|
| `parallel` | 所有請求獨立執行 | 批次操作 |
| `takeLatest` | 新請求自動取消前一個請求 | 即時搜尋 |
| `dedupe` | 相同 key 的後續呼叫共享既有 Promise | 組件掛載 |
| `serial` | 請求排隊依序執行 | 表單提交 |
...
const msg = resolveNetMessage('net.timeout', 'zh-TW')
// => '網絡超時，請稍後再試'
```

支援語系：`en` / `ja` / `zh-TW` / `zh-CN` / `zh-HK`

## Moyin 生態系成員

本模組是 [**Moyin Factory**](https://github.com/AtsushiHarimoto/Moyin-Factory) 的一部分。
